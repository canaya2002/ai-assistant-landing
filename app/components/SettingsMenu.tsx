...
export default function SettingsMenu({ isOpen, onClose }: SettingsMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div 
        ref={menuRef}
        className="absolute right-4 top-16 w-80 bg-black/90 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center justify-between">
            <h3 className="text-white font-medium font-lastica">Configuración</h3>
            <button
              onClick={onClose}
        ...
        <div className="p-2">
          ...
          {(plan === 'pro' || plan === 'pro_max') && (
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center space-x-3 px-4 py-3 text-white hover:bg-white/10 rounded-xl transition-all duration-300 mb-2"
            >
